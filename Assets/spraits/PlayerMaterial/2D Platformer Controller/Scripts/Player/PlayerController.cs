...
namespace SupanthaPaul
{
	public class PlayerController : MonoBehaviour
	{
			...
			private void Update()
		{
			...
			{
				// Мышь справа, поворачиваемся вправо
		...
		void ShootDagger()
		{
			...
			Vector3 mousePosition = Camera.main.ScreenToWorldPoint(Input.mousePosition);
			mousePosition.z = 0; // Убираем координату Z

			// Вычисляем направление от игрока до мыши
			Vector3 direction = (mousePosition - transform.position).normalized;

			// Если игрок смотрит влево, инвертируем направление по X
			if (!isFacingRight)
			{
				direction.x = -direction.x;
			}

			// Создаем экземпляр кинжала
			GameObject dagger = Instantiate(knights, kpoint.transform.position, Quaternion.identity);
